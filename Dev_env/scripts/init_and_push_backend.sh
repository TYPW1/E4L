...
GIT_REMOTE_REPO="http://typw01:Pierr01999.@192.168.56.10/gitlab/typw01/lu.uni.e4l.platform.frontend.dev"
...
if ! git remote | grep -q front; then
    git remote add front "$GIT_REMOTE_REPO"
else
    git remote set-url front "$GIT_REMOTE_REPO"
fi
